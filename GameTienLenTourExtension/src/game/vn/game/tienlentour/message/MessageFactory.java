 ...
 */
package game.vn.game.tienlentour.message;
...
import game.vn.game.tienlentour.TienLenTourController;
...
public class MessageFactory {
     ...
     * @return
     */
    public SFSObject createMessageStopGame(String userIdCurrentPlayer, TienLenPlayer[] players, TienLenTourController game) {
        ...
        for (int i = 0; i < players.length; i++) {
            ...
            if (user != null) {
                ...
                playerData.putUtfString("curUid",game.getIdDBOfUser(user));
                playerData.putDouble("penalty", players[i].getPenalty());
     ...
     * @return
     */
    public SFSObject createMessageFinishGame(String userId, byte typeWin, byte winTime) {
        ...
        finishObj.putByte("winTime", winTime);
        return finishObj;
    }
     ...
     * @param currentUser
     * @param isChatHeo
     * @param sizeCardOfMover
     * @return
     */
    public SFSObject createMessageMove(String userIdMover, List<Short> cardMove, String currentUser,boolean isChatHeo,byte sizeCardOfMover) {
        SFSObject sfso = new SFSObject();
        ...
        sfso.putByte("sizeCards", sizeCardOfMover);
        return sfso;
    }
     ...
     * @return
     */
    public SFSObject createReturnMessage(TienLenTourController controller, String userMover, Card[] cardMove, String currentUser, List<Card> cardsMyself,
            int getTimeRemain, boolean isUserBeginNewRound, TienLenPlayer[] players, int playingTime, double bonusMoney, int countWinMax) {
        SFSObject sfso = new SFSObject();
        ...
        for (int i=0; i<players.length; i++) {
            ...
            if (u != null) {
                SFSObject playerData = new SFSObject();
                playerData.putUtfString("userId", Utils.getIdDBOfUser(u));
                playerData.putBool("isSkip", players[i].isSkipstatus());
                playerData.putByte("sizeCard", (byte) players[i].getCards().size());
                playerData.putByte("winCount", controller.getWinsOfUser(u));
                sfsArrUsers.addSFSObject(playerData);
            }
        }
        ...
        sfso.putDouble("bonusMoney", bonusMoney);
        sfso.putByte("countWinMax", (byte)countWinMax);
        return sfso;
    }
    /**
